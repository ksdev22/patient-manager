export default function PatientList({
  ...
  return (
    <div>
      <div className="bg-white z-[95] grid grid-flow-col font-bold px-5 border-b-2 border-black select-none min-w-max sticky top-0 left-0 h-[10vh]">
        {filters.columns.map((column) => {
          if (column.checked) {
            return (
              <div
                key={column.name}
                className="w-40 border-l-2 pl-3 text-center flex items-center space-x-5"
              >
                <div>
